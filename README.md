...
- **Links para o Flask (cadastro, recuperar senha, quem somos):** no `index.html` e `index-1.html` use a meta **`fmp-backend-origin`** (padrão `http://127.0.0.1:5050`, igual ao `SPA_PORT` do `run_stack.py`). Os links apontam para `/cadastro`, `/recuperar_senha` e `/quem_somos` nesse servidor. No GitHub Pages, altere o `content` da meta para a URL pública do seu backend, se houver.
